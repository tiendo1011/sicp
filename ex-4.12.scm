...
; the new abstract procedure
(define (env-loop env var handle-not-found-in-first-frame-proc handle-found-in-first-frame-proc)
  (if (eq? env the-empty-environment)
      (error "Unbound variable" var)
      (let* ((frame (first-frame env))
             (var-index (find-index (frame-variables frame) var)))
            (if (eq? var-index -1)
                (handle-not-found-in-first-frame-proc (enclosing-environment env) frame)
                (handle-found-in-first-frame-proc frame var-index)))))

(define (lookup-variable-value var env)
  (define
    handle-found-in-first-frame-proc
    (lambda (frame var-index) (list-ref (frame-values frame) var-index)))
  (define
    handle-not-found-in-first-frame-proc
    (lambda (enclosing-env first-frame)
      (env-loop enclosing-env var handle-not-found-in-first-frame-proc handle-found-in-first-frame-proc)))
  (env-loop env var handle-not-found-in-first-frame-proc handle-found-in-first-frame-proc))

(define (set-variable-value! var val env)
  (define
    handle-found-in-first-frame-proc
    (lambda (frame var-index) (set-list-ref (frame-values frame) var-index val)))
  (define
    handle-not-found-in-first-frame-proc
    (lambda (enclosing-env first-frame)
      (env-loop enclosing-env var handle-not-found-in-first-frame-proc handle-found-in-first-frame-proc)))
...
(define (define-variable! var val env)
  ...
  (define
    handle-not-found-in-first-frame-proc
    (lambda (enclosing-env first-frame)
      (add-binding-to-frame! var val first-frame)))
  (env-loop env var handle-not-found-in-first-frame-proc handle-found-in-first-frame-proc))
